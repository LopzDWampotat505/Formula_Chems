...
public class Escuderia {
    ...
    public Escuderia(String n, Vehiculo v1, Vehiculo v2, List<Driver> drivers){
        ...
        if(drivers.contains(null)){
            throw new IllegalArgumentException("La lista de pilotos de una escudería no debe de tener pilotos inexistentes");
        }
    ...
    public void addDriver(Driver d) {
        if (d == null) throw new IllegalArgumentException("El piloto no puede ser inexistente.");
        if (this.listaDrivers.size() >= MAX_PILOTOS) throw new IllegalStateException("El límite máximo es " + MAX_PILOTOS);
        this.listaDrivers.add(d);
    }

    public String getNombre(){ return nombre; }
    public List<Vehiculo> getVehiculos() { return new ArrayList<>(this.listaVehiculos); }
    public List<Driver> getDrivers() { return new ArrayList<>(this.listaDrivers); }
}
